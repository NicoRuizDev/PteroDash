...
db.prepare(
  `CREATE TABLE IF NOT EXISTS "keyv" ("key" VARCHAR(255) PRIMARY KEY, "value" TEXT)`
).run();

module.exports = {
  async get(key) {
    let row = await db
      .prepare("SELECT * FROM keyv WHERE key = ?")
      .get(`keyv:${key}`);
  ...
  async set(key, value) {
    ...
    await db.prepare("INSERT INTO keyv (key, value) VALUES (?, ?)").run(
      `keyv:${key}`,
      JSON.stringify({
        value: value,
        expires: null,
