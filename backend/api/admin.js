...
module.exports.load = async function (app, db) {
  app.get("/setcoins", async (req, res) => {
    ...
    if (
      ...
      !newsettings.api.client.webhook.auditlogs.disabled.includes("ADMIN")
    ) {
      ...
      fetch(`${newsettings.api.client.webhook.webhook_url}`, {
        ...
        console.warn(
          chalk.red("[WEBSITE] There was an error sending to the webhook: " + e)
        )
      );
    }
  });
    ...
    if (req.query.ram || req.query.disk || req.query.cpu || req.query.servers) {
      ...
      if (
        ...
        fetch(`${newsettings.api.client.webhook.webhook_url}`, {
          ...
          console.warn(
            chalk.red(
              "[WEBHOOK] There was an error sending a message to the webhook:\n" +
                e
            )
          )
        );
  ...
  app.get("/setplan", async (req, res) => {
    ...
    if (!req.query.package) {
      ...
      if (
        newsettings.api.client.webhook.auditlogs.enabled === true &&
        !newsettings.api.client.webhook.auditlogs.disabled.includes("ADMIN")
      ) {
        let id = req.query.id;
        let username = cacheaccountinfo.attributes.username;
  ...
  app.get("/userinfo", async (req, res) => {
    ...
    if ((await userinforeq.statusText) == "Not Found") {
      console.log(
        "[WEBSITE] An error has occured while attempting to get a user's information"
      );
