  ...
  if (settings.api.client.allow.renewsuspendsystem.enabled == true) {
    ...
    app.get("/renew", async (req, res) => {
      ...
      if (newsettings.api.client.allow.overresourcessuspend == true) {
        ...
        for (
          let i = 0, len = userinfo.relationships.servers.data.length;
          i < len;
          i++
        ) {
          current.ram =
            current.ram +
            userinfo.relationships.servers.data[i].attributes.limits.memory;
          current.disk =
            current.disk +
            userinfo.relationships.servers.data[i].attributes.limits.disk;
          current.cpu =
            current.cpu +
            userinfo.relationships.servers.data[i].attributes.limits.cpu;
        }
        ...
        if (
          ...
          return res.send(
            theme.settings.redirect.failedrenewserver + "?err=EXCEEDSPLAN"
          );
        }
      }
      ...
      if (usercoins < cost)
        return res.redirect(
          theme.settings.redirect.failedrenewserver + "?err=CANNOTAFFORD"
        );
      ...
      renewalservers[req.query.id] =
        settings.api.client.allow.renewsuspendsystem.time;
