...
module.exports.load = async function (app, db) {
  ...
  app.get(settings.api.client.oauth2.callbackpath, async (req, res) => {
    ...
    if (!req.query.code)
      return res.redirect(failedcallback + "?err=MISSINGCODE");
    ...
    if (json.ok == true) {
      ...
      if (missingscopes.length !== 0)
        return res.redirect(
          failedcallback +
            "?err=MISSINGSCOPES&scopes=" +
            missingscopes.join("%20")
        );
      ...
      if (userinfo.verified == true) {
        ...
        if (newsettings.api.client.oauth2.ip.block.includes(ip))
          return res.redirect(failedcallback + "?err=IPBLOCKED");
        ...
        if (newsettings.api.client.j4r.enabled == true) {
          if (guildsinfo.message == "401: Unauthorized")
            return res.send(
              "Please allow us to know what servers you are in to let the J4R system work properly."
            );
        ...
        if (newsettings.api.client.bot.joinguild.enabled == true) {
          if (typeof newsettings.api.client.bot.joinguild.guildid == "string") {
            ...
            if (
              checkmemberexistjson.message &&
              checkmemberexistjson.message === "Unknown Member" &&
              settings.api.client.bot.joinguild.forcejoin
            )
              return res.redirect(failedcallback + "?err=DISCORD");
            ...
            if (Array.isArray(newsettings.api.client.bot.joinguild.guildid)) {
              ...
              return res.send(
                "Tell an administrator there is an error settings.json: api.client.bot.joinguild.guildid is not an array nor a string."
              );
            }
          } else {
            return res.send(
          ...
          if (newsettings.api.client.allow.newusers == true) {
            ...
            if ((await accountjson.status) == 201) {
              ...
              let user = accountlist.data.filter(
                (acc) =>
                  acc.attributes.email.toLowerCase() ==
        ...
        if (
          newsettings.api.client.webhook.auditlogs.enabled &&
          !newsettings.api.client.webhook.auditlogs.disabled.includes("LOGIN")
        ) {
          let params = JSON.stringify({
            embeds: [
              {
                title: "Login",
                description: `**__User:__** ${req.session.userinfo.username}#${
                  req.session.userinfo.discriminator
                } (${req.session.userinfo.id}) \n**IP:** ${
                  newsettings.api.client.oauth2.ip["duplicate check"] == true
                    ? await db.get("ip-" + req.session.userinfo.id)
                    : "IP Checking is off"
                }`,
          ...
          fetch(`${newsettings.api.client.webhook.webhook_url}`, {
            ...
            console.warn(
              chalk.red(
                "[WEBSITE] There was an error sending to the webhook: " + e
              )
            )
          );
        }
