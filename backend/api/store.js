...
module.exports.load = async function (app, db) {
  app.get("/buyram", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (extra.ram > 999999999999999)
        return res.redirect(failedcallback + "?err=MAXIMUMRAM");
  ...
  app.get("/buydisk", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (extra.disk > 999999999999999)
        return res.redirect(failedcallback + "?err=MAXIMUMDISK");
  ...
  app.get("/buycpu", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (extra.cpu > 999999999999999)
        return res.redirect(failedcallback + "?err=MAXIMUMCPU");
  ...
  app.get("/buyservers", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (extra.servers > 999999999999999)
        return res.redirect(failedcallback + "?err=MAXIMUMSERVERS");
      ...
      res.redirect(
        (theme.settings.redirect.purchaseservers || "/") + "?err=none"
      );
    }
  });
