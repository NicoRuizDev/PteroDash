...
module.exports.load = async function (app, db) {
  app.get("/coupon_redeem", async (req, res) => {
    ...
    if (!code)
      return res.redirect(
        theme.settings.redirect.missingorinvalidcouponcode +
          "?err=MISSINGCOUPONCODE"
      );
    ...
    */
    ...
    res.redirect(
      theme.settings.redirect.successfullyredeemedcoupon +
        "?err=SUCCESSCOUPONCODE"
    );
    ...
    if (
      ...
      !newsettings.api.client.webhook.auditlogs.disabled.includes(
        "COUPONREDEEM"
      )
    ) {
